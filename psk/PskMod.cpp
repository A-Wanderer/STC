...
MatrixControl* PskMod::getMatrixControl(){
  static MatrixControl control;
  return &control;
}
...
int PskMod::modulate(int max_num, std::string &data, std::string& out){
  string cmd = "python3 Pskmod.py modulate " + to_string(max_num) + " " + data;
...
int PskMod::deModulate(int max_num, std::string &data, std::string& out){
  string cmd = "python3 Pskmod.py demodulate " + to_string(max_num) + " " + data;
  return runCmd(cmd, out);
}
/*
